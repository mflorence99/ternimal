...
const chmoder = (descs: FileDescriptor[], chmod: Chmod): void => {
  async.filter(descs, async desc => {
    ...
    if (descs.length !== successes.length) {
      const failures =
        descs.filter(desc => !successes.find(success => success.path === desc.path));
      report(failures);
...
const report = (failures: FileDescriptor[]): void => {
  ...
  if (failures.length > 2)
    message += ` and ${failures.length - 1} others`;
...
ipcMain.on(Channels.fsChmod, (_, descs: FileDescriptor[], chmod: Chmod): void => {
  chmoder(descs, chmod);
});
